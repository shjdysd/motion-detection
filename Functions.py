...
def drawRectangle(img, originalImg, op_flow, threshold):
    ...
    for leader in DJSet.group:
        if len(DJSet.group[leader]) < 2: continue
        frame = [leader[0],leader[1],leader[0],leader[1]]
        for member in DJSet.group[leader]:
            if member[0] < frame[0]: frame[0] = member[0]
            elif member[0] > frame[2]: frame[2] = member[0]
            if member[1] < frame[1]: frame[1] = member[1]
            elif member[1] > frame[3]: frame[3] = member[1]
        if frame[3] - frame[1] < 50 or  frame[2] - frame[0] < 50:
            continue;
